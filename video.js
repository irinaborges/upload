...
router.get('/:slug', async function (req, res) {
    let video = await query("select id, titulo as title, categoria as categories, url_youtube as video, canal as channel_id, miniatura as logo, data_publicacao as date, descricao as description FROM video WHERE slug = ?", [req.params.slug]);
    res.json(video);
});
